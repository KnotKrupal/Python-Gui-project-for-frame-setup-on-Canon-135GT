...
class PDFBuilder:
    ...
    def _generate_outlines(self, progress: ProgressCallback = None) -> None:
        ...
        try:
            ...
            if progress:
                progress(35, "Outlines ready")
        finally:
            doc.close()

    def _generate_artwork(self, progress: ProgressCallback = None) -> None:
        doc = fitz.open()
        try:
            page = doc.new_page(width=self.layout.page_width, height=self.layout.page_height)
    ...
    def _place_logo(
        ...
        try:
            ...
            if self.params.flip_in_app:
                transform = fitz.Matrix(-1, 0, 0, 1, logo_page.rect.width, 0) * transform
            if rotate_degrees:
                transform = fitz.Matrix(-1, 0, 0, -1, logo_page.rect.width, logo_page.rect.height) * transform
