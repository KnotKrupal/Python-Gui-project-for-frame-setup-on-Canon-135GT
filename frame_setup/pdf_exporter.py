...
class PDFBuilder:
    ...
    def generate_pdfs(
        ...
        *,
        ...
        if export_artwork:
            if not export_outlines and progress:
                progress(5, "Preparing artwork")
            elif progress:
                progress(45, "Preparing artwork")
            ...
            try:
                ...
                if not self.params.allow_raster_fallback:
                    raise
                if progress:
                    progress(80, "Vector placement failed – rasterising logo")
                self._export_artwork_raster(placements)
            artwork_path = self.params.artwork_path

        if progress:
            progress(100, "Finished")
        return ExportResult(outline_path, artwork_path)
