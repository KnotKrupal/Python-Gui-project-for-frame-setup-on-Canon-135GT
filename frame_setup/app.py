...
class FrameSetupApp(ttk.Frame):
    ...
    def schedule_preview_update(self) -> None:
        canvas_widget = getattr(self, "preview_canvas", None)
    ...
    def update_preview(self) -> None:
        ...
        width = max(int(canvas_widget.winfo_width()), int(float(canvas_widget.cget("width"))))
        height = max(int(canvas_widget.winfo_height()), int(float(canvas_widget.cget("height"))))
        ...
        try:
            ...
            self._render_preview_message(canvas_widget, width, height, "Enter numeric values")
        ...
        if glass_width <= 0 or glass_height <= 0 or bed_width_in <= 0:
            self._render_preview_message(canvas_widget, width, height, "Enter positive dimensions")
        ...
        canvas_widget.create_rectangle(bed_left, bed_top, bed_right, bed_bottom, outline="#4c5c78", width=1.4)

        seam_y = bed_bottom - glass_height * scale
        arrangement_right = bed_left + total_clusters_width * scale
        canvas_widget.create_line(bed_left, seam_y, arrangement_right, seam_y, fill="#9fa8b8", dash=(4, 3))
        ...
        for cluster_index in range(cluster_count):
            ...
            for row in range(2):
                for column in range(2):
                    ...
                    canvas_widget.create_rectangle(left, top, right, bottom, fill=frame_fills[row], outline="#7a879a", width=1)
                    ...
                    if visible_band > 0:
                        ...
                        if band_bottom - band_top > 2:
                            canvas_widget.create_rectangle(left + inset, band_top, right - inset, band_bottom, fill=band_color, outline="")
                    ...
                    if has_logo:
                        ...
                        if logo_right - logo_left > 2 and logo_bottom - logo_top > 2:
                            ...
                            canvas_widget.create_rectangle(logo_left, logo_top, logo_right, logo_bottom, outline=outline_color, width=1.4, fill="#ffffff")
                    ...
                    canvas_widget.create_polygon(points, fill=arrow_color, outline="")
